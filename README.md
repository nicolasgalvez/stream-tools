...
└── stream_tools_mcp/       # MCP server for Claude Code / Cursor
    └── server.py           # 27 tools wrapping all library services
scripts/
...
```bash
# Project-scoped (shared with the team via .mcp.json)
...
If you are running from source without a global install, use the module entry point:
...
Auth uses the same OAuth chain as the CLI (env vars, then cached token, then browser). Run `yt auth login` first to authenticate.
